...
export default class VideoMaster extends Component {
    ...
    componentDidMount(){
        ...
        axios.get("http://localhost:3001/school/get-all-video").then((res) => {
    ...
    handleuplaod = (e) => {
        ...
        axios.post("http://localhost:3001/school/add-video-image", data).then((res) => {
            console.log(res);
        })
    }
    ...
    onSubmit = () => {
        console.log(this.state);
        axios.post("http://localhost:3001/school/add-video",{faculty : this.state.faculty, standard_id : this.state.standard_id, subject_id : this.state.subject_id, chapter_master_id : this.state.chapter_master_id, video_link : this.state.video_link, video_name : this.state.video_name})
            .then((res)=>{
                console.log(res);
    ...
    deleteRecord(id) {
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    axios.delete(`http://localhost:3001/school/delete-video/${id}`).then((res) => {
                        this.componentDidMount();
                    })
                } else {
  ...
  render() {
    return (
        <div>
                    ...
                    <div className="spContainer mx-auto">
                        <div className="card px-4 py-5 border-0 shadow">
                            <div className="d-inline text-left mb-3">
                                <h3 className="font-weight-bold">Video Master</h3>
                            </div>
                            ...
                            <div className="d-inline text-center mb-0">
                                <div className="form-group mx-auto">
                                    <input className="form-control inpSp" type="text" name='video_link' placeholder="Video Link" onChange={this.handleonchange} />
                                </div>
                            </div>

                            <div className="d-inline text-center mb-0">
                                <div className="form-group mx-auto">
                                    <input className="form-control inpSp" type="text" name='video_name' placeholder="Video Name" onChange={this.handleonchange} />
                                </div>
                            </div>
            ...
            <button type="button" class="btn btn-warning" data-toggle="modal" data-target=".bd-example-modal-lg">See Video List</button>

                <div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h3 class="modal-title">Video List</h3>
                                        ...
                                        {this.state.all.map((x, key) => {
                                            return (
                                                <tr>
                                                    ...
                                                    <td>{x.video_link}</td>
                                                    <td>{x.video_image}</td>
                                                    <td><button type="button" class="btn btn-warning" onClick={() => { this.deleteRecord(x.videoMaterial_id) }}>Delete</button></td>
                                                </tr>
                                            )
                                        })}
                                    </tbody>
